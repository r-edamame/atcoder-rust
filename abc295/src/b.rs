...
fn main() {
    ...
    let mut ctoi = "0123456789".chars().collect::<Vec<char>>();
    ...
    for y in 0..r {
        for x in 0..c {
            if mat[y][x].is_digit(10) {
                bs.push(((x, y), ctoi.iter().position(|&c| c == mat[y][x]).unwrap()))
            }
        }
    }
...
fn len(x: usize, y: usize) -> usize {
    return ((x as isize) - (y as isize)).abs() as usize;
}
