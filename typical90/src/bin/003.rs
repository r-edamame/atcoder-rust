    ...
    let (f1, d1) =
        bfs((0, 0),
            ...
            |(p, d), r| {
                ...
                g[p].iter().map(move |q| (*q, d+1) ).collect::<Vec<(usize, usize)>>()
            }
        );
    let (_, d2) =
        bfs((f1, 0),
            ...
            |(p, d), r| {
                ...
                g[p].iter().map(|q| (*q, d+1) ).collect::<Vec<(usize, usize)>>()
            }
        );
...
fn bfs<F, V: Copy + Display + Debug, R>(s: (usize, V), mut r: R, f: F) -> R
    where F: Fn((usize, V), &mut R) -> Vec<(usize, V)>
{
    let mut passed = BSet::new();
    let mut queue = Queue::new();
    queue.push_back(s);
    while let Some(t) = queue.pop_front() {
        if !passed.insert(t.0) {
            continue;
        }
        for v in f(t, &mut r) {
            queue.push_back(v);
