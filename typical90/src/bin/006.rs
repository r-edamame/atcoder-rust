...
fn main() {
    ...
    let mut pss = vec![vec![-1isize; 26]; n];
    pss[n-1][chars.iter().position(|c| *c == s[s.len()-1]).unwrap()] = n as isize -1;
    for i in (0..n-1).rev() {
        let j = chars.iter().position(|c| *c == s[i]).unwrap();
        for l in 0..26 {
            pss[i][l] = if j == l { i as isize } else { pss[i+1][l] };
        }
    }
    ...
    for i in 0..k {
        for c in 0..26 {
            let p = pos as usize;
            if pss[p][c] != -1 && (pss[p][c] as usize) + ((k-i)-1) < n {
                ans.push(chars[c]);
                pos = pss[p][c] + 1;
                break;
            }
