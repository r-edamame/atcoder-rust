...
fn nearest<V: Copy + PartialOrd + Sub<Output = V>>(v: &Vec<V>, t: V) -> V {let mut cs = vec![];if let Some(ii) = bsearch(0, v.len() as isize - 1, |i| t < v[i as usize]) {let i = ii as usize;if i == 0 {cs.push(v[i]);cs.push(v[v.len() - 1]);} else {cs.push(v[i]);cs.push(v[i - 1])}} else {cs.push(v[0]);cs.push(v[v.len() - 1]);}return *cs.iter().min_by(|&&a, &&b| {abs_diff(t, a).partial_cmp(&abs_diff(t, b)).unwrap()}).unwrap();}
...
fn main() {
    ...
    for i in 0..n {
        ...
        for j in 0..n-1 {
            ...
            let n = (a - nearest(&angles, (a+180.0).min(180.0-a))).abs();
            ans = ans.max(n.min(360.0-n));
        }
    }
...
fn angle(y: f64, x: f64) -> f64 {
    let a = y.atan2(x) * 180f64 / PI;
    return a.min(360f64 - a);
}