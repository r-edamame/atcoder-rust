...
fn main() {
    ...
    let mut digits = bit_digits(1<<n).collect::<Vec<usize>>();
    digits.reverse();
    for x in v {
        println!("{}", digits.iter().map(|i| if x & i > 0 { '(' } else { ')' } ).collect::<String>());
    }
}

fn valid(pat: usize, l: usize) -> bool {
    let mut c = 1;
    for i in bit_digits(1<<l) {
        c += if pat & i > 0 { -1 } else { 1 };
        if c == 0 {
            return false;
        }
    }
