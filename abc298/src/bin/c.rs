...
fn main() {
    ...
    let mut sets = vec![BSet::new(); 2*100000];
    let mut boxes = vec![BSet::new(); 2*100000];
    ...
    for k in 0..q {
        ...
        match c {
            ...
            2 => {
                ...
                let vs = boxes[i].iter().map(|(v, _) | v+1).collect::<Vec<usize>>();
                print_vec(vs);
            },
            3 => {
                input! {
                ...
                let vs = sets[i].iter().map(|&v| v+1).collect::<Vec<usize>>();
                print_vec(vs);
            }
            _ => panic!()
        }
...
fn print_vec(v: Vec<usize>) {
    println!("{}", v.iter().map(|i| i.to_string()).join(" "));
}