...
fn main() {
    ...
    let center = 10000 / 2 + 1;

    if ix.abs() as usize > center || iy.abs() as usize > center {
        yesno(false);
        return;
    }

    let cast = |i: &isize| { (*i + center as isize) as usize };

    let x = cast(&ix);
    let y = cast(&iy);

    let hor = d.iter().step_by(2).map(|i| *i as usize).collect::<Vec<usize>>();
    let ver = d.iter().skip(1).step_by(2).map(|i| *i as usize).collect::<Vec<usize>>();

    let mut dp1 = vec![vec![false; 10002]; 1001];
    dp1[1][center + hor[0]] = true;
    for i in 2..=hor.len() {
        for p in 0..=10000 {
            if dp1[i-1][p] {
                if p + hor[i-1] <= 10001 {
                    dp1[i][p + hor[i-1]] = true;
                }
                if p >= hor[i-1] {
                    dp1[i][p - hor[i-1]] = true;
                }
    ...
    let mut dp2 = vec![vec![false; 10002]; 1001];
    dp2[0][center] = true;
    for i in 1..=ver.len() {
        for p in 0..=10000 {
            if dp2[i-1][p] {
                if p + ver[i-1] <= 10001 {
                    dp2[i][p + ver[i-1]] = true;
                }
                if p >= ver[i-1] {
                    dp2[i][p - ver[i-1]] = true;
                }
    ...
    yesno(dp1[hor.len()][x] && dp2[ver.len()][y]);
}
