...
fn main() {
    ...
    let mut v = Vec::from_iter(ss.iter().zip(0..));
    v.sort_by_key(|t| t.0);

    let mut results = vec![0; n];
    for i in 0..n {
        if i > 0 {
            results[v[i].1] = lcp(v[i].0, v[i+1].0);
        }
        if i < n-1 {
            results[v[i].1] = results[v[i].1].max(lcp(v[i].0, v[i-1].0));
        }
    }
    println!("{}", results.iter().map(|x| x.to_string()).collect::<Vec<String>>().join("\n"));
}

fn lcp(s: &Vec<char>, t: &Vec<char>) -> usize {
    let mut r = 0;
