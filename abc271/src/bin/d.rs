...
fn main() {
    ...
    let mut dp = vec![vec![(vec![], vec![]); s+1]; n+1];
    dp[0][0].0.push((0, 0));
    dp[0][0].1.push((0, 0));
    ...
    let has = dp[n][s].0.len() + dp[n][s].1.len() > 0;
    yesno(has);
    ...
    if has {
        ...
        let mut t = s;
        for d in (2..=n).rev() {
            if *flips.last().unwrap() == 0 {
                flips.push(dp[d][t].0[0].0);
                t = dp[d][t].0[0].1;
            } else {
                flips.push(dp[d][t].1[0].0);
                t = dp[d][t].1[0].1;
            }
        }
        println!("{}", String::from_iter(flips.iter().rev().map(|f| if *f == 0 { 'H' } else { 'T' })));
    }
}
