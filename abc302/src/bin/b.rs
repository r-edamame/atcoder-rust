        ...
        for y in 0..h {
            ...
            for dx in vec![-1, 0, 1] {
                for dy in vec![-1, 0, 1] {
                    ...
                    let b = (1..5).all(|i| {
                        let iy = y as isize + (dy * i);
                        let ix = x as isize + (dx * i);

                        if iy < 0 || h as isize <= iy || ix < 0 || w as isize <= ix {
                            return false;
                        }
                        return ss[iy as usize][ix as usize] == ['s','n','u','k','e'][i as usize]
                    });

                    if b {
                        for i in 0..5 {
                            println!("{} {}", 1 + y as isize + i * dy, 1 + x as isize + i * dx)
                        }
                        return;
                    }
                }
