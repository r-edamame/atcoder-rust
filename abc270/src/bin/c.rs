...
fn ndg(n: usize, es: &Vec<(usize, usize)>) -> Vec<Vec<usize>> {
    let mut g = vec![vec![]; n];
    for &(u, v) in es {
        g[u].push(v);
        g[v].push(u);
...
fn main() {
    ...
    parents[x] = x as isize;
    queue.push_back(x);
    while let Some(p) = queue.pop_front() {
        for &q in g[p].iter() {
            if parents[q] == -1 {
                queue.push_back(q);
                parents[q] = p as isize;
            }
        }
    }
    ...
    let mut p = y;
    while p != x {
        path.push(parents[p] as usize);
        p = parents[p] as usize;
    }
    println!("{}", path.iter().rev().map(|i| (i+1).to_string()).collect::<Vec<String>>().join(" "));
}
