...
use std::collections::{VecDeque as Queue, HashSet as HSet, BTreeSet as BSet, BTreeMap as BMap, BinaryHeap};
...
fn main() {
    ...
    let mut dp = vec![vec![std::usize::MAX; n]; k+1];
    let mut set = BSet::new();
    for &a in al.iter() {
        sized_push(k, &mut heap, &mut set, a);
    }
    for i in 0..n {
        dp[0][i] = 0;
    }
    ...
    for i in 1..=k {
        for j in 0..n {
            for l in 0..n {
                ...
                let c = a + al[j];
                // println!("c: {}", c);
                sized_push(k, &mut heap, &mut set, c);
                dp[i][j] = dp[i][j].min(c);
            }
        }
    }
...
fn sized_push(k: usize, h: &mut BinaryHeap<usize>, s: &mut BSet<usize>, a: usize) {
    if s.contains(&a) {
        return;
    }
